  ...
  try {
    ...
    bridge_1.on(DepositEvent, async (tokenAddress, depositor, amount, event) => {
      ...
      try {
        ...
        const isValidStoredProof = await oracle_2.verifyMerkleProof(proofFromTxData, txHash, leafFromTxData);
        ...
        ], "0xFE90951bD9Ae68d27e9BD1224051A3e4A2b8a651");


        const wETHbalance = await wETH.connect(validator_2).balanceOf(DEPOSITOR);
        ...
        const wETHbalanceAFTER = await wETH.connect(validator_2).balanceOf(DEPOSITOR);
