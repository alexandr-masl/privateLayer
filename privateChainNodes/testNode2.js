...
const validator_1 = new ethers.Wallet(TestValidatorKey, provider1);
const validator_2 = new ethers.Wallet(process.env.Validator_1, provider2);
...
async function main() {
  try {
    ...
    const oracle_1 = new ethers.Contract(OracleDepositStorage.network1, JSON.parse(fs.readFileSync('./artifacts/contracts/OracleDepositStorage.sol/OracleDepositStorage.json')).abi, validator_1);

    console.log(colors.green('Listening for events on Chain 2...'));

    bridge_2.on(DepositEvent, async (tokenAddress, depositor, amount, event) => {

      const txHash = event.log.transactionHash;
      console.log(colors.yellow(`\n\nDeposit from Chain 2 detected: tokenAddress=${tokenAddress}, depositor=${depositor}, amount=${amount}, txHash=${txHash}`));
      console.log(`Validator:`, validator_2.address);
      ...
      try {
        ...
        const tx = await oracle_1.submitDepositHeader(txHash, root, leafNodes);
        await tx.wait();
        ...
        const isValidStoredProof = await oracle_1.verifyMerkleProof(proofFromTxData, txHash, leafFromTxData);
        ...
        const wETHbalance = await wETH.connect(validator_1).balanceOf(DEPOSITOR);
        ...
        const wETHbalanceAFTER = await wETH.connect(validator_1).balanceOf(DEPOSITOR);
