...
const fs = require('fs');
const { Bridge, Chain1_RPC, Chain2_RPC, OracleDepositStorage, DepositEvent } = require('./nodesConfig.json');
...
async function main() {
  try {
    // Initialize contract instances for Bridge on Chain 1 and OracleDepositStorage on Chain 2
    ...
    bridge1.on(DepositEvent, async (tokenAddress, depositor, amount, event) => {
      ...
      // Submit Merkle root and proof to OracleDepositStorage contract on Chain 2
      const tx = await oracle2.submitDepositHeader(txHash, root, leafNodes);
      await tx.wait();
